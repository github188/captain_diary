-- 17.07.05 19:00 我加一条  金额设计为bigint ，页面前端用自己用js转成元，或元转分
...
（24）禁止使用INSERT INTO t_xxx VALUES(xxx)，必须显示指定插入的列名
...
如果用了关系型数据库，用表的join是很自然的做法，除非一些特例：sql语句太复杂，或统计信息不准确，造成数据库生成的执行计划不正确，导致运行效率低下，且短时间没办法改写sql
